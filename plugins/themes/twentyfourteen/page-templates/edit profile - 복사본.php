...
<?php
	...
	if (isset($_POST)) {
		...
		// 	echo "<div class='pmpro_error'> Please complete all required fields which is highlighted in Red.</div>";
		// }
	...
	<div id="primary" class="content-area" style="margin-left: 15%;width: 50%;">
		...
		<form action="" method="post" enctype="multipart/form-data">			
			...
			jQuery(document).ready(function () {					
				...
				var companyType = "<?php echo get_user_meta($current_user->ID, 'company_type', true); ?>";
				...
				var schoolName = "<?php echo get_user_meta($current_user->ID, 'school_name', true); ?>";
				if (schoolName == "Other"){
				...
				function addBarAdmission(){    
					...
					+ "<td style='float: left;' class='name_Table'><input type='text' name='ba_date[]' placeholder='mm/dd/yyyy' /></td>"
		...
		<label for="public_directory">
			<input name="public_directory" type="checkbox" id="public_directory" <?php checked( get_user_meta($current_user->ID, 'pmpromd_hide_directory', true), NULL); ?> value="1">
		...
		<table class="name_Table">
			<tr class="name_Table">
				<td id="first_name_div" class="pmpro_checkout-field name_Table">
					...
					<input type="text" id="first_name" name="first_name" value="<?php echo esc_attr(get_user_meta($current_user->ID, 'first_name', true)); ?>" size="20" class="input <?php echo pmpro_getClassForField("first_name");?>">									    
				</td>			
											
				<td id="Last Name_div" class="pmpro_checkout-field name_Table">
					<label for="last_name">Last Name</label>
					<input type="text" id="last_name" name="last_name" value="<?php echo esc_attr(get_user_meta($current_user->ID, 'last_name', true)); ?>" size="20" class="input <?php echo pmpro_getClassForField("last_name");?>">									    
				</td>				
			</tr>
			...
			<tr class="name_Table">
				<td id="pref_name_div" class="pmpro_checkout-field name_Table">
					...
					<input type="text" id="pref_name" name="pref_name" value="<?php echo esc_attr(get_user_meta($current_user->ID, 'pref_name', true)); ?>" size="20" class="input <?php echo pmpro_getClassForField("pref_name");?>">							    
				</td>
			</tr>								
		...
		<table class="name_Table" id="workContact">	
			<tr class="name_Table">
				<td id="company_name_div" class="pmpro_checkout-field name_Table">
					...
					<input type="text" id="company_name" name="company_name" value="<?php echo esc_attr(get_user_meta($current_user->ID, 'company_name', true)); ?>" size="20" class="input <?php echo pmpro_getClassForField("company_name");?>">							    
				</td>
			</tr>								
			...
			<tr class="name_Table">
				<td id="position_div" class="pmpro_checkout-field name_Table">
					...
					<input type="text" id="position" name="position" value="<?php echo esc_attr(get_user_meta($current_user->ID, 'position', true)); ?>" size="20" class="input <?php echo pmpro_getClassForField("position");?>">
				</td>
			</tr>			
			...
			<tr class="name_Table">
				...
				<td id="otherTxtWork" class="pmpro_checkout-field name_Table" style="visibility:hidden;">
					...
					<input type="text" id="company_other" name="company_other" value="<?php echo esc_attr(get_user_meta($current_user->ID, 'company_other', true)); ?>" size="20" class="input <?php echo pmpro_getClassForField("company_other");?>">				
				</td>
			</tr>		
		...
		<table class="name_Table" id="schoolContact" style="display:none;">
			<tr class="name_Table">
				...
				<td id="otherTxtSchool" class="pmpro_checkout-field name_Table" style="visibility:hidden;">
					...
					<input type="text" id="school_other" name="school_other" value="<?php echo esc_attr(get_user_meta($current_user->ID, 'school_other', true)); ?>" size="20" class="<?php echo pmpro_getClassForField("school_other");?>">
				</td>			
			</tr>
			<tr class="name_Table">
				<td id="graduation_date_div" class="pmpro_checkout-field name_Table">
					<label for="graduation_date">Expected Graduation Year</label>
					<input type="text" id="graduation_date" name="graduation_date" size="20" value="<?php echo esc_attr(get_user_meta($current_user->ID, 'graduation_date', true)); ?>" class="input <?php echo pmpro_getClassForField("graduation_date");?>" >			    
				</td>
			</tr>
		</table>
		...
		<table class="name_Table">
						...
						<?php 	
								...
								if (count($ba_area_arr) > 0){
									for ($i = 0; $i < count($ba_area_arr); $i++){
										?>
										<tr class="name_Table">
											<td style="float: left; margin-right: 5%; margin-left: 2%;" class="name_Table"><input type="text" name="ba_area[]" value="<?php echo esc_attr($ba_area_arr[$i]); ?>" /></td>
											<td style="float: left;" class="name_Table"><input type="text" name="ba_date[]" value="<?php echo esc_attr($ba_date_arr[$i]); ?>" /></td>
											<td class="name_Table" style="float: left; margin-left: 3%; margin-top: -0.5%;"><span class="btnDelete" style="font-size: 25px; cursor: pointer;" onclick="deleteBarAdmission()">x</span></td>
										</tr> 
										<?php
									}
			...
			<tr class="name_Table" style="padding:10%">
			</tr>
							...
							<?php 	
								...
								if (count($lang_name_arr) > 0){
									for ($i = 0; $i < count($lang_name_arr); $i++){
										?>
										<tr class="name_Table">
											<td style="float: left; margin-right: 5%; margin-left: 2%;" class="name_Table"><input type="text" name="lang_name[]" value="<?php echo esc_attr($lang_name_arr[$i]); ?>" /></td>
											<td style="float: left; margin-top: 1%; width: 30%;" class="name_Table">
												<select name="lang_level[]">
													<option value="">Select Level</option>
													<?php 	
			...
			<tr class="name_Table">
				<td id="practice_areas_div" class="pmpro_checkout-field name_Table">
					<label for="practice_areas">Practice Area(s)</label>
			...
			<tr class="name_Table">
				<td id="public_profile_div" class="pmpro_checkout-field name_Table">
					<label for="public_profile">Public Profile <span style="font-size:75%;">(URL; one of facebook, linkedin, lawfirm website and etc)<span></label>
