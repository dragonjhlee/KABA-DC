...
class Theme_My_Login_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $title ) {
			echo $args['before_title'] . $title . " - " . $current_user->user_login . $args['after_title'] ;
		}
		...
		if ( is_user_logged_in() ) :
			 ...
			 */
			$profile_url = "http://" . $_SERVER['SERVER_NAME'] . "/membership-account/edit-profile";
			...
			<div class="tml tml-user-panel">
				<?php if ( ! empty( $avatar_size ) ) : ?>
					...
					<?php
						$profile_pic_url = get_user_meta(get_current_user_id(), 'profile_pic_url', true);													
						if (!empty($profile_pic_url)){
					?>
						<a href="<?php echo $profile_url; ?>"> 
							<img src="<?php echo esc_attr($profile_pic_url); ?>" width="100" height="100" />
						</a>
					<?php 
						} else { 
						echo get_avatar(get_current_user_id(), $avatar_size);
					}
					?>
				<?php endif; ?>
