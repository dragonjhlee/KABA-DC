...
<div class="ahb-section-container">
	<div class="ahb-section ahb-section-active" data-step="1">
        ...
        <p class="cpmvcontainer"><?php _e('The payment form was placed into the page','cp-contact-form-with-paypal'); ?> <a href="<?php echo $cp_contactformpp_postURL; ?>"><?php echo $cp_contactformpp_postURL; ?></a>.</p>
        <p class="cpmvcontainer"><?php _e('Now you can','cp-contact-form-with-paypal'); ?>:</p>
        <div style="clear:both"></div>
        <button class="button button-primary cpmvcontainer" type="button" id="nextBtn" onclick="window.open('<?php echo $cp_contactformpp_postURL; ?>');"><?php _e('View the Published Form','cp-contact-form-with-paypal'); ?></button>
        <div style="clear:both"></div>
...
<?php
  ...
  foreach ($myrows as $item)            
      echo '<option value="'.$item->id.'"'.($item->id==$_GET["cal"]?' selected':'').'>'.$item->form_name.'</option>';
?>                
            </select>
                    </td>    
                </tr>   
                ...
                <tr valign="top"  id="ppage" style="display:none">
					...
					<td valign="top">
                       ...
                       <div style="border: 1px dotted black; background-color: #FFFACD ;padding:15px; font-weight: bold; margin:10px;">
                         [CP_CONTACT_FORM_PAYPAL id="<?php echo (intval(@$_GET["cal"])?intval(@$_GET["cal"]):'1'); ?>"]
                       </div>
