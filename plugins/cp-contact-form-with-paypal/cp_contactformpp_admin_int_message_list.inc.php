...
<script type="text/javascript">
 ...
 {    
    document.location = 'admin.php?page=cp_contact_form_paypal.php&cal=<?php echo $_GET["cal"]; ?>&list=1&status='+status+'&lu='+id+'&r=<?php echo $nonce; ?>';   
 } 
 function cp_deleteMessageItem(id)
 {
    ...
    {        
        document.location = 'admin.php?page=cp_contact_form_paypal.php&cal=<?php echo $_GET["cal"]; ?>&list=1&ld='+id+'&r=<?php echo $nonce; ?>';
    }
 }
 ...
 function cp_deleteall()
 {
    ...
    {        
        document.location = 'admin.php?page=cp_contact_form_paypal.php&cal=<?php echo intval(CP_CONTACTFORMPP_ID); ?>&list=1&del=all&r='+Math.random();
    }    
 }
 function cp_markall()
 {
 ...
 {    
    if (confirm('<?php _e('Are you sure you want to refund this item? Note that this action cannot be undone.','cp-contact-form-with-paypal'); ?>'))
        document.location = 'admin.php?page=cp_contact_form_paypal.php&cal=<?php echo intval(CP_CONTACTFORMPP_ID); ?>&list=1&refund=1&lur='+id+'&r=<?php echo $nonce; ?>';   
 }   
</script>
<div class="wrap">
...
<div id="normal-sortables" class="meta-box-sortables">
 <hr />
 <h3><?php _e('This message list is from','cp-contact-form-with-paypal'); ?>: <?php if (CP_CONTACTFORMPP_ID != 0) echo $myform[0]->form_name; else echo 'All forms'; ?></h3>
</div>
...
<form action="admin.php" method="get">
 ...
 <nobr><?php _e('Search for','cp-contact-form-with-paypal'); ?>: <input type="text" name="search" value="<?php echo esc_attr($_GET["search"]); ?>" /> &nbsp; &nbsp; &nbsp;</nobr> 
 <nobr><?php _e('From','cp-contact-form-with-paypal'); ?>: <input type="text" id="dfrom" name="dfrom" value="<?php echo esc_attr($_GET["dfrom"]); ?>" /> &nbsp; &nbsp; &nbsp; </nobr>
 <nobr><?php _e('To','cp-contact-form-with-paypal'); ?>: <input type="text" id="dto" name="dto" value="<?php echo esc_attr($_GET["dto"]); ?>" /> &nbsp; &nbsp; &nbsp; </nobr>
 <nobr><?php _e('Item','cp-contact-form-with-paypal'); ?>: <select id="cal" name="cal">
...
echo paginate_links(  array(
    'base'         => 'admin.php?page=cp_contact_form_paypal.php&cal='.CP_CONTACTFORMPP_ID.'&list=1%_%&dfrom='.urlencode($_GET["dfrom"]).'&dto='.urlencode($_GET["dto"]).'&search='.urlencode($_GET["search"]),
    'format'       => '&p=%#%',
    'total'        => $total_pages,
	  ...
	  <tr class='<?php if (!($i%2)) { ?>alternate <?php } ?>author-self status-draft format-default iedit' valign="top">
		...
		<td class="noprint">
		  <?php if ($events[$i]->paid == 1) { ?>
   	        <input type="button" name="calmanage_<?php echo $events[$i]->id; ?>" value="<?php _e('Change status to NOT PAI','cp-contact-form-with-paypal'); ?>D" onclick="cp_updateMessageItem(<?php echo $events[$i]->id; ?>,0);" /><br />      
            <input type="button" name="calrefund_<?php echo $events[$i]->id; ?>" value="<?php _e('Refund','cp-contact-form-with-paypal'); ?>" onclick="cp_refundItem(<?php echo $events[$i]->id; ?>,1);" /><br /> 
 		  <?php } else { ?>
 		    <input type="button" name="calmanage_<?php echo $events[$i]->id; ?>" value="<?php _e('Change status to PAID','cp-contact-form-with-paypal'); ?>" onclick="cp_updateMessageItem(<?php echo $events[$i]->id; ?>,1);" /><br />                             
 		  <?php } ?>          
		  
		  <input type="button" name="caldelete_<?php echo $events[$i]->id; ?>" value="<?php _e('Delete','cp-contact-form-with-paypal'); ?>" onclick="cp_deleteMessageItem(<?php echo $events[$i]->id; ?>);" />                             
		</td>
      </tr>
     <?php } ?>
	</tbody>
