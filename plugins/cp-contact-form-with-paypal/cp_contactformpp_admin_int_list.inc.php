...
<script type="text/javascript">
 function cp_addItem()
 {
    ...
    document.location = 'admin.php?page=cp_contact_form_paypal.php&a=1&r=<?php echo $nonce; ?>&name='+encodeURIComponent(calname);       
 }
 
 function cp_updateItem(id)
 {
    var calname = document.getElementById("calname_"+id).value;    
    document.location = 'admin.php?page=cp_contact_form_paypal.php&u='+id+'&r=<?php echo $nonce; ?>&name='+encodeURIComponent(calname);    
 }
 
 function cp_cloneItem(id)
 {
    document.location = 'admin.php?page=cp_contact_form_paypal.php&c='+id+'&r=<?php echo $nonce; ?>';
 } 
 
 function cp_manageSettings(id)
 {
    document.location = 'admin.php?page=cp_contact_form_paypal.php&cal='+id+'&r=<?php echo $nonce; ?>';
 }
 ...
 {
    document.location = 'admin.php?page=cp_contact_form_paypal.php&cal='+id+'&list=1&r=<?php echo $nonce; ?>';
 } 
 ...
 function cp_deleteItem(id)
 {
    ...
    {        
        document.location = 'admin.php?page=cp_contact_form_paypal.php&d='+id+'&r=<?php echo $nonce; ?>';
    }
 }
 ...
 function cp_updateConfig()
 {
    ...
    {        
        ...
        document.location = 'admin.php?page=cp_contact_form_paypal.php&ac=st&scr='+scr+'&chs='+chs+'&r='+Math.random();
    }    
 }
...
?>
   <tr> 
    ...
    <td>
                             <input style="margin-bottom:5px;" class="button"  type="button" name="calupdate_<?php echo $item->id; ?>" value="<?php _e('Rename','cp-contact-form-with-paypal'); ?>" onclick="cp_updateItem(<?php echo $item->id; ?>);" />
                             <input style="margin-bottom:5px;" class="button-primary button"  type="button" name="calmanage_<?php echo $item->id; ?>" value="<?php _e('Settings','cp-contact-form-with-paypal'); ?>" onclick="cp_manageSettings(<?php echo $item->id; ?>);" />
                             <input style="margin-bottom:5px;" class="button-primary button"  type="button" name="calmanage_<?php echo $item->id; ?>" value="<?php _e('Publish','cp-contact-form-with-paypal'); ?>" onclick="cp_publish(<?php echo $item->id; ?>);" />
                             <input style="margin-bottom:5px;" class="button"  type="button" name="calmanagem_<?php echo $item->id; ?>" value="<?php _e('Messages','cp-contact-form-with-paypal'); ?>" onclick="cp_viewMessages(<?php echo $item->id; ?>);" />                     
                             <input style="margin-bottom:5px;" class="button"  type="button" name="calclone_<?php echo $item->id; ?>" value="<?php _e('Clone','cp-contact-form-with-paypal'); ?>" onclick="cp_cloneItem(<?php echo $item->id; ?>);" />                             
                             <input style="margin-bottom:5px;" class="button"  type="button" name="caldelete_<?php echo $item->id; ?>" value="<?php _e('Delete','cp-contact-form-with-paypal'); ?>" onclick="cp_deleteItem(<?php echo $item->id; ?>);" />                             
    </td>
    <td nowrap><nobr>[CP_CONTACT_FORM_PAYPAL id="<?php echo $item->id; ?>"]</nobr></td>          
   </tr>
<?php  
   } 
?>   
...
<div class="ahb-section-container">
 ...
 <div id="metabox_basic_settings" class="postbox" >
  ...
  <div class="inside"> 
      <p><?php _e('Use this area to add custom CSS styles or custom scripts. These styles and scripts will be keep safe even after updating the plugin.','cp-contact-form-with-paypal'); ?></p>
      <input type="button" onclick="cp_editArea('css');" name="gobtn3" value="<?php _e('Add Custom Styles','cp-contact-form-with-paypal'); ?>" />
      &nbsp; &nbsp; &nbsp;      
      <input type="button" onclick="cp_editArea('js');" name="gobtn2" value="<?php _e('Add Custom JavaScript','cp-contact-form-with-paypal'); ?>" />
  </div>    
 </div> 
