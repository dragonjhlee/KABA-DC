...
if ( 'POST' == $_SERVER['REQUEST_METHOD'] && isset( $_POST[$cpid.'_post_edition'] ) )
    echo "<div id='setting-error-settings_updated' class='updated settings-error'> <p><strong>".__('Settings saved.','cp-contact-form-with-paypal')."</strong></p></div>";
...
<input type="button" name="backbtn" value="<?php _e('Back to items list','cp-contact-form-with-paypal'); ?>..." onclick="document.location='admin.php?page=<?php echo $plugslug; ?>';">
<br /><br />
...
<?php if ($_GET["item"] == 'css') { ?>
    <?php _e('You can add custom CSS rules into this area (NO need to place the &lt;style&gt;&lt;/style&gt; tags). For ideas about useful styles check the','cp-contact-form-with-paypal'); ?> <a href="https://cfpaypal.dwbooster.com/faq#design"><?php _e('design section of the FA','cp-contact-form-with-paypal'); ?>Q</a>.<br /><br />
<?php } else if ($_GET["item"] == 'js') { ?>
     <?php _e('You can add custom scripts into this area (NO need to place the &lt;script&gt;&lt;/script&gt; tags).','cp-contact-form-with-paypal'); ?><br /><br />
<?php }  ?>
