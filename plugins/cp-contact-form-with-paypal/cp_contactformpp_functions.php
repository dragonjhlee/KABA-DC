...
function _cp_contactformpp_install() {
    ...
    $sql = "CREATE TABLE $table_name (
         ...
         paypalexpress_api_username varchar(250) DEFAULT '' NOT NULL,
         paypalexpress_api_password varchar(250) DEFAULT '' NOT NULL,
         paypalexpress_api_signature varchar(250) DEFAULT '' NOT NULL,
         enable_paypal_expresscredit_yes varchar(250) DEFAULT '' NOT NULL,
         enable_paypal_expresscredit_no varchar(250) DEFAULT '' NOT NULL,
...
function cp_contactformpp_gutenberg_block() {
    ...
    wp_deregister_script('query-cfppstringify');
    wp_register_script('query-cfppstringify', plugins_url('/js/jQuery.stringify.js', __FILE__));
    ...
    wp_enqueue_script( 'cp_contactformpp_builder_script',
           plugins_url('/js/fbuilder.jquery.js', __FILE__),array("jquery","jquery-ui-core","jquery-ui-datepicker","jquery-ui-widget","jquery-ui-position","jquery-ui-tooltip","query-cfppstringify","cp_contactformpp_validate_script"), false, true ); 
...
<script type="text/javascript">
 ...
 function doValidate<?php echo $CP_CPP_global_form_count; ?>(form)
 {
    ...
    {
        setTimeout( "cfwpp_cerror<?php echo $CP_CPP_global_form_count; ?>()", 100);
        return false;
    }
    ...
    {
        var cpefb_error = 0;
        $dexQuery("#cp_contactformpp_pform<?php echo $CP_CPP_global_form_count; ?>").find(".cpefb_error").each(function(index){
...
function cfwpp_cerror<?php echo $CP_CPP_global_form_count; ?>(){$dexQuery = jQuery;$dexQuery("#hdcaptcha_error<?php echo $CP_CPP_global_form_count; ?>").css('top',$dexQuery("#hdcaptcha_cp_contact_form_paypal_post<?php echo $CP_CPP_global_form_count; ?>").outerHeight());$dexQuery("#hdcaptcha_error<?php echo $CP_CPP_global_form_count; ?>").css("display","inline");}
...
function set_cp_contactformpp_insert_adminScripts($hook) {
    if (isset($_GET["page"]) && $_GET["page"] == "cp_contact_form_paypal.php")
    {
        ...
        wp_register_script('query-stringify', plugins_url('/js/jQuery.stringify.js', __FILE__));
        wp_enqueue_script( 'cp_contactformppv2_buikder_script', plugins_url('/js/fbuilder.jquery.js', __FILE__),array("jquery","jquery-ui-core","jquery-ui-sortable","jquery-ui-dialog","jquery-ui-tabs","jquery-ui-droppable","jquery-ui-button","jquery-ui-datepicker","query-stringify") );
...
function cp_contact_form_paypal_check_posted_data() {
    ...
    if ( 'POST' == $_SERVER['REQUEST_METHOD'] && isset( $_POST['CP_CFWPP_post_edition'] ) && is_admin() )
    {
        cp_contactformpp_save_edition();
        return;
    }  
...
<input type="hidden" name="business" value="<?php echo trim(cp_contactformpp_get_option('paypal_email', CP_CONTACTFORMPP_DEFAULT_PAYPAL_EMAIL)); ?>" />
...
<input type="hidden" name="a1" value="<?php echo $paypal_recurrent; ?>">
<input type="hidden" name="p1" value="<?php echo ($recurrent_fp=='0.4'?'1':$recurrent_fp); ?>">
...
<?php if ($selnum != '0') { ?>
<input type="hidden" name="srt" value="<?php echo $selnum; ?>">
<?php } ?>
<input type="hidden" name="no_note" value="1" />
<input type="hidden" name="cmd" value="_xclick-subscriptions">
<input type="hidden" name="bn" value="NetFactorSL_SI_Custom">
<input type="hidden" name="a3" value="<?php echo cp_contactformpp_apply_tax($price, $taxes); ?>">
<input type="hidden" name="p3" value="<?php echo ($recurrent=='0.4'?'1':$recurrent); ?>">
...
<input type="hidden" name="return" value="<?php echo esc_url(trim(cp_contactformpp_get_option('fp_return_page', CP_CONTACTFORMPP_DEFAULT_fp_return_page))); ?>">
<input type="hidden" name="cancel_return" value="<?php echo esc_url($error_page); ?>" />
<input type="hidden" name="currency_code" value="<?php echo cp_contactformpp_clean_currency(cp_contactformpp_get_option('currency', CP_CONTACTFORMPP_DEFAULT_CURRENCY)); ?>" />
<input type="hidden" name="lc" value="<?php echo trim(cp_contactformpp_get_option('paypal_language', CP_CONTACTFORMPP_DEFAULT_PAYPAL_LANGUAGE)); ?>" />
<input type="hidden" name="notify_url" value="<?php echo cp_contactformpp_get_FULL_site_url(); ?>/?cp_contactformpp_ipncheck=<?php echo $item_number; ?>" />
</form>
<script type="text/javascript">
...
function cp_contactformpp_check_IPN_verification() {
    ...
    $_GET["itemnumber"] = $_GET['cp_contactformpp_ipncheck'];
    ...
    if ($myrows[0]->paid == 0)
    {
        $params["txnid"] = $_POST['txn_id'];
        $wpdb->query("UPDATE ".CP_CONTACTFORMPP_POSTS_TABLE_NAME." SET paid=1,paypal_post='".esc_sql($str)."',posted_data='".esc_sql(serialize($params))."' WHERE id=".intval($_GET['itemnumber']));
...
function cp_contactformpp_check_IPN_verification_express() {
	...
	{	
	    ...
	    $products[0]['ItemPrice'] = $params["Cost"]; //Item Price
    ...
    header('Location: '.cp_contactformpp_get_option('fp_return_page', CP_CONTACTFORMPP_DEFAULT_fp_return_page));
    exit();
}
...
function cp_contactformpp_export_csv ()
{
    ...
    for ($i=0; $i<$end; $i++)
    {
        $hlabel = iconv("utf-8", "ISO-8859-1//TRANSLIT//IGNORE", cp_contactformpp_get_field_name($fields[$i],@$form_data[0]));
...
function cp_contactformpp_translate_json($str)
{
    ...
    {
        $form_data[0][$i]->title = __($form_data[0][$i]->title,'cp-contact-form-with-paypal');   
        $form_data[0][$i]->userhelpTooltip = __($form_data[0][$i]->userhelpTooltip,'cp-contact-form-with-paypal'); 
...
function cp_contactformpp_save_edition()
{
    foreach ($_POST as $item => $value)
        if (!is_array($value))
            $_POST[$item] = stripcslashes($value);    
    ...
    if ($_POST["cfwpp_edit"] == 'js')   
        update_option('CP_CFWPP_JS', base64_encode($_POST["editionarea"]));  
    else if ($_POST["cfwpp_edit"] == 'css')  
        update_option('CP_CFWPP_CSS', base64_encode($_POST["editionarea"]));  
}
...
function cp_contactformpp_save_options()
{
    ...
    $data = array(
                  ...
                  'fp_error_page' => $_POST['fp_error_page'],
                  ...
                  'paypalexpress_api_username' => @$_POST["paypalexpress_api_username"],
                  'paypalexpress_api_password' => @$_POST["paypalexpress_api_password"],
                  'paypalexpress_api_signature' => @$_POST["paypalexpress_api_signature"],
                  'enable_paypal_expresscredit_yes' => @$_POST["enable_paypal_expresscredit_yes"],
                  'enable_paypal_expresscredit_no' => @$_POST["enable_paypal_expresscredit_no"],                  
                  
                  'pprefunds' => @$_POST["pprefunds"],
                  ...
                  'donationlayout' => $_POST["donationlayout"],
                  ...
                  'paypal_recurrent_fp' => $_POST["paypal_recurrent_fp"],
                  'paypal_recurrent_setup' => $_POST["paypal_recurrent_setup"],
                  'paypal_recurrent_times' => $_POST["paypal_recurrent_times"],
...
function cp_contactformpp_data_management_loaded() 
{
    ...
    $action = @$_POST['cp_contactformpp_do_action_loaded'];
	if (!$action) return; // go out if the call isn't for this one

    if ($_POST['cp_contactformpp_id']) $item = $_POST['cp_contactformpp_id'];

    if ($action == "wizard")
    {
        $shortcode = '[CP_CONTACT_FORM_PAYPAL id="'.$item .'"]';
        $cp_contactformpp_postURL = cp_contactformpp_publish_on(@$_POST["whereto"], @$_POST["publishpage"], @$_POST["publishpost"], $shortcode, @$_POST["posttitle"]);            
        return;
    }
