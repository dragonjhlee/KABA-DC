...
global $first_name, $last_name, $pref_name, $sec_email, $pmpro_required_user_profile, $pmpro_required_user_profile_work, $pmpro_required_user_profile_school; // KABA
...
//echo 'pmpro_level : ' . $pmpro_level . '<br/>';

if ( empty( $pmpro_level->id ) ) {
	echo 'pmpro_level Error : ' . pmpro_url( "levels" ) . '<br/>';
...
if ( isset( $_REQUEST['pref_name'] ) ) {
	$pref_name = sanitize_text_field( $_REQUEST['pref_name'] );
} else {
	$pref_name = "";
}
...
if ( isset( $_REQUEST['company_type'] ) ) {
	...
	if ($company_type == "Other" && isset( $_REQUEST['company_other'] )){
		$company_other = sanitize_text_field( $_REQUEST['company_other'] );
	}
} else {
	$company_type = "";
}
if ( isset( $_REQUEST['company_size'] ) ) {
...
if ( isset( $_REQUEST['school_name'] ) ) {
	...
	if ($school_name == "Other" && isset( $_REQUEST['school_other'] )){
		$school_other = sanitize_text_field( $_REQUEST['school_other'] );
	}
} else {
	$school_name = "";
}
if ( isset( $_REQUEST['graduation_date'] ) ) {
	$graduation_date = sanitize_text_field( $_REQUEST['graduation_date'] );
} else {
	$graduation_date = "";
}
...
if ( $submit && $pmpro_msgt != "pmpro_error" ) {
	...
	$isValid = true;
	if ( ! empty( $_REQUEST['IsLawStudent'] ) ) {		
	...
	if ( ! $current_user->ID ) {
		...
		*/
		...
		if ( ! empty( $_REQUEST['sec_email'] ) ) {
			$sec_email = sanitize_email($_REQUEST['sec_email']);
		} else {
			$sec_email = "";
		}	
		...
		update_usermeta($user_id, "pref_name", $pref_name);
		update_usermeta($user_id, "sec_email", $sec_email);
		update_usermeta($user_id, "prm_phone", $prm_phone);
		update_usermeta($user_id, "sec_phone", $sec_phone);
		...
		if ( ! empty( $_REQUEST['IsLawStudent'] ) ) {
			...
			if ( ! empty( $_REQUEST['school_name'] ) ) {
				$school_name = $_REQUEST['school_name'];
				if ($school_name == "Other" &&  ! empty( $_REQUEST['school_other'] )){
					$school_other = sanitize_text_field($_REQUEST['school_other']);
					update_usermeta($user_id, "school_other", $school_other);
				}
			} else {
				$school_name = "";
			}			
			...
			update_usermeta($user_id, "school_name", $school_name);
			update_usermeta($user_id, "graduation_date", $graduation_date);	
			...
			if ( ! empty( $_REQUEST['company_type'] ) ) {
				$company_type = $_REQUEST['company_type'];
				if ($company_type == "Other" && ! empty( $_REQUEST['company_other'] )){
					$company_other = sanitize_text_field($_REQUEST['company_other']);
					update_usermeta($user_id, "company_other", $company_other);
				}
			} else {
				$company_type = "";
			}				
			if ( ! empty( $_REQUEST['company_size'] ) ) {
				$company_size = $_REQUEST['company_size'];
			} else {
				$company_size = "";
			}
			if ( ! empty( $_REQUEST['company_city'] ) ) {
				$company_city = sanitize_text_field($_REQUEST['company_city']);
			} else {
				$company_city = "";
			}
			if ( ! empty( $_REQUEST['company_state'] ) ) {
				$company_state = $_REQUEST['company_state'];
			} else {
				$company_state = "";
			}		

			update_usermeta($user_id, "company_name", $company_name);
			update_usermeta($user_id, "position", $position);
			update_usermeta($user_id, "company_type", $company_type);
			update_usermeta($user_id, "company_size", $company_size);
			update_usermeta($user_id, "company_city", $company_city);
			update_usermeta($user_id, "company_state", $company_state);
		}
		update_usermeta($user_id, "is_law_student", $is_law_student);		
		
		// More 
		// Bar Admission
		$ba_count = count($_REQUEST['ba_area']);
		if($ba_count > 0) {
			$item_count = 0;			
			$ba_area = "";
			$ba_date = "";
			for($i=0; $i<$ba_count; $i++) {
				if(!empty($_REQUEST["ba_area"][$i]) || !empty($_REQUEST["ba_date"][$i])) {
					$item_count++;
					if($ba_area != "") {
						$ba_area .= ",";
					}
					$ba_area .= $_REQUEST["ba_area"][$i];
					if($ba_date != "") {
						$ba_date .= ",";
					}
					$ba_date .= $_REQUEST["ba_date"][$i];
				}
			}
			if($item_count != 0) {
		...
		if ( ! empty( $_REQUEST['kor_proficiency'] ) ) {
			$kor_proficiency = $_REQUEST['kor_proficiency'];
		} else {
			$kor_proficiency = "";
		}
		...
		$lang_count = count($_REQUEST['lang_name']);
		if($lang_count > 0) {
			$item_count = 0;			
			$lang_name = "";
			$lang_level = "";
			for($i=0; $i<$lang_count; $i++) {
				if(!empty($_REQUEST["lang_name"][$i]) || !empty($_REQUEST["lang_level"][$i])) {
					$item_count++;
					if($lang_name != "") {
						$lang_name .= ",";
					}
					$lang_name .= $_REQUEST["lang_name"][$i];
					if($lang_level != "") {
						$lang_level .= ",";
					}
					$lang_level .= $_REQUEST["lang_level"][$i];
				}
			}
			if($item_count != 0) {
		...
		// More Law Schools	
		$more_school_count = count($_REQUEST['more_school_name']);
			if ($more_school_count > 0) {
				$item_count = 0;
				$more_school_name = "";
				$more_school_other = "";
				for($i = 0; $i < $more_school_count; $i++) {
					if (!empty($_REQUEST["more_school_name"][$i])) {
						$item_count++;
						if ($more_school_name != "") {
							$more_school_name .= ";";
						}
						$more_school_name .= $_REQUEST["more_school_name"][$i];
						
						if ( $i > 1) {
							$more_school_other .= ",";
						}
						$more_school_other .= $_REQUEST["more_school_other"][$i];		
					}
				}
				if ($item_count > 0){
		...
		// if ( ! empty( $_REQUEST['more_school_name'] ) ) {
		// 	$more_school_name = $_REQUEST['more_school_name'];
		// 	if ($more_school_name == "Other" && ! empty( $_REQUEST['more_school_other'] )){
		// 		$more_school_other = sanitize_text_field($_REQUEST['more_school_other']);
		// 		update_usermeta($user_id, "more_school_other", $more_school_other);
		// 	}
		// } else {
		// 	$more_school_name = "";
		...
		if ( ! empty( $_REQUEST['graduation_year'] ) ) {
			$graduation_year = sanitize_text_field($_REQUEST['graduation_year']);
		} else {
			$graduation_year = "";
		}
		...
		if ( ! empty( $_REQUEST['area_other'] ) ) {
			$area_other = sanitize_text_field($_REQUEST['area_other']);
		} else {
			$area_other = "";
		}
	

		update_usermeta($user_id, "kor_proficiency", $kor_proficiency);			
		//update_usermeta($user_id, "more_school_name", $more_school_name);		
		update_usermeta($user_id, "graduation_year", $graduation_year);
		update_usermeta($user_id, "practice_areas", $practice_areas);
		update_usermeta($user_id, "public_profile", $public_profile);	
		update_usermeta($user_id, "area_other", $area_other);
